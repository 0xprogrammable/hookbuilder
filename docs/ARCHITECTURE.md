...
| Public Applicant intake | Root `submissions/` schema, example and validator | Review-only pull requests to `0xprogrammable/hookbuilder` that bind one exact public source revision, declared launch wallet, `custom-graph@1.0.0` on chain `1`, deterministic path and canonical manifest digest; no Registry, Router or launch write |
