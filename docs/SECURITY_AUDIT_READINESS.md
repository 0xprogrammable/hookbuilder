...
| Agent evaluation harness | The repository gate dynamically discovers 9 local test source files covering the encrypted sealed-after-design corpus, hard gates, fresh repositories, workspace/symlink mutation, wrapper closure and caller-policy distrust; the count is not an execution receipt and no real model run was made | Real provider/model and judge receipts, trusted sandbox, comparable public repository E2Es, independent novel holdout/prior comparator, pinned fork, and installed-host behavior remain `EXTERNAL_BLOCKED` |
