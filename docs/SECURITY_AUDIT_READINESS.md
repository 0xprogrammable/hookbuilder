...
| Open-World V2 schemas, V4 semantics, security, composition, routing, and Project Compiler | Focused local suites exercise no-market absence, standard-v4/canonical-adapter contracts, fail-closed Node/Python/shell/PATH/symlink/network cases, exact external-receipt subject/policy/output bindings and trusted Ed25519 verification. The portable builder executes no candidate bytes; static legacy receipts remain unauthenticated and `NOT_PROVEN` | Trusted external sandbox authority/integration plus independent adversarial validator, compiler, adapter, and protocol review pending |
...
| Agent evaluation harness | The repository gate dynamically discovers 10 local test source files covering the encrypted sealed-after-design corpus, hard gates, fresh repositories, workspace/symlink mutation, wrapper closure and caller-policy distrust; the count is not an execution receipt and no real model run was made | Real provider/model and judge receipts, trusted sandbox, comparable public repository E2Es, independent novel holdout/prior comparator, pinned fork, and installed-host behavior remain `EXTERNAL_BLOCKED` |
