...
Private maintainer record for the Programmable v4 Builder `v0.4.0` candidate. This document records what was inspected,
...
The Builder should let an agent turn an ordinary or unfamiliar product idea into the smallest composable Uniswap v4
project, test the parts that can be checked deterministically, preserve novel ideas for architecture review, and prepare
one exact GitHub application revision. It should know enough protocol, SDK, routing, liquidity, security, Programmable
economics, packaging, and review workflow to avoid rebuilding standard machinery from memory.
...
## Official source snapshot inspected
...
The catalog keeps ordinary launch, custom hook, and blank custom foundations and composes capability packs on top. New
...
Templates are planning accelerators, not the set of allowed products. An unlisted capability stays intact, receives a
blank custom plan and an architecture-review route, and is never marked unsafe merely because a classifier does not
recognize it.

### Programmable platform contract

- Every launch-ready canonical pool enforces the inclusive Programmable 10 bps volume share in its one hook.
...
- GitHub is the only current application transport. The builder can prepare, submit after an exact external-write
  confirmation, update, and read status, but cannot approve itself.
...
| --- | --- |
| Mode-only exploration | Intake, fee policy and template catalog only |
| Ordinary launch preflight | About 17,000 estimated tokens; no game, SDK or advanced-hook chapters |
...
| Unlisted idea | Keeps the idea eligible and adds scenario, surface, Lego and security chapters for architecture review |
...
remain separate exceptions. The frozen `v0.3.0` private bundle is never rewritten; this work produces a new `v0.4.0`
candidate.
