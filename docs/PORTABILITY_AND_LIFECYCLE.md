...
The bundled v0.12.0 candidate reports `publicationStateVerified: false`; this document alone does not prove that the tag,
release, marketplace, or installed bytes are public or live.

## Host matrix for the v0.12.0 candidate package
...
| Application V3 exact revision | 22+ | 2.49+ with bounded `git fetch --stdin` | No | Yes | Declared supported when doctor gates pass | Declared supported when doctor gates pass | Unsupported; fails closed |
...
| GitHub submit/update | 22+ | 2.49+ with bounded `git fetch --stdin` | Authenticated | Yes | Declared supported after explicit authorization | Declared supported after explicit authorization | Unsupported for exact-revision flow |
...
The latest verified public GitHub Release is `v0.11.0`.

After independently confirming the exact public tag and GitHub release, preview the immutable release before installation:
...
The v0.12.0 candidate package contains the versioned Codex marketplace and payload, but it is not public installation
proof. Until its release closes, public plugin use stays on the exact reviewed `v0.11.0` release. A rollback still
requires an exact previously reviewed tag and a fresh plugin validation; never repoint to a mutable branch.
