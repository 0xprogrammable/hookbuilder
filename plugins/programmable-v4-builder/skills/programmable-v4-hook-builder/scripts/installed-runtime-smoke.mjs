...
// This compact runtime smoke suite is intentionally shipped. The complete
// development suite lives outside the portable package at test/portable-skill.
...
test("host-neutral version reports the bundled standalone release without state", () => {
  ...
  assert.equal(result.status, 0, result.stdout || result.stderr);
  const output = JSON.parse(result.stdout);
  assert.equal(output.result.installed.releaseVersion, "0.10.1");
