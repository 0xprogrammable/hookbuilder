...
test("module syntax validation batches candidate files without executing them", () => {
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "programmable-syntax-batch-"));
  const markerPath = path.join(fixtureRoot, "executed");
  const validPath = path.join(fixtureRoot, "valid.mjs");
  const invalidPath = path.join(fixtureRoot, "invalid.mjs");
  ...
  try {
    ...
    fs.writeFileSync(invalidPath, "export const = ;\n");

    const findings = validateModuleSyntax([
      { label: "scripts/valid.mjs", path: validPath },
      { label: "scripts/invalid.mjs", path: invalidPath }
    ]);

    assert.equal(findings.length, 1);
    assert.match(findings[0], /^scripts\/invalid\.mjs:/u);
