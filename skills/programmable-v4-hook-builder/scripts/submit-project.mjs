...
function inspectSource(repositoryRoot) {
  try {
...
function resolveWorkspace(repositoryRoot, input) {
  const requested = input === null
    ? defaultWorkspacePath(repositoryRoot)
    : input;
...
function discoverTrackedFiles(repositoryRoot, commit, basename) {
...
function loadApplicantPackagePointer(repositoryRoot, pointerPath, workspace) {
  ...
  if (
    ...
    || value.kind !== "programmable-applicant-package-pointer"
  ) {
...
function bindApplicationSources(application, repositoryRoot, workspace, source) {
  ...
  for (const repository of repositories) {
    ...
    try {
      const observed = inspectSource(candidate);
  ...
  if (missing.length > 0 || roots.length !== repositories.length) {
    throw new CliFailure(
      "MULTI_REPOSITORY_WORKSPACE_INCOMPLETE",
      `exact local source roots are missing for: ${missing.slice(0, 8).join(", ")}`,
      { exitCode: 1 }
    );
  }
  return roots;
...
function loadApplicationPackageSnapshot(applicationPackage) {
  try {
...
function loadApplicationDraftSource(draftPath) {
...
function sourceRootArgs(sourceRoots) {
...
function isRealDirectoryWithFile(directory, filename) {
  try {
...
function isExactKind(target, kind) {
  try {
    const stat = fs.lstatSync(target);
