...
This default reports the constants bundled with the checked-out Builder. The v0.12.0 candidate package reports
`channel: stable`, `publicationState: release-package`, and `publicationStateVerified: false`; these bundled constants
do not resolve or authenticate a public tag. Public installation guidance remains pinned to the verified v0.11.0
GitHub Release until a later release and installed bytes are independently verified. To inspect
...
| Release candidate | `assets/templates/release-candidate.example.json` | Binds the planned 0.12.0 transition but deliberately leaves source and release-artifact coordinates incomplete |
...
fixture set. It tests generic update and migration behavior; it is not the current `0.12.0` candidate record. Never copy their trust key, pin
...
0.12.0 planning example it declares:

- Builder `0.11.2 -> 0.12.0` with semantic classification `minor`;
