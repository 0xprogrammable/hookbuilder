...
This default reports the constants bundled with the checked-out Builder. The v0.9.1 release package reports
`channel: stable`, `publicationState: release-package`, and `publicationStateVerified: false`; these bundled constants
do not resolve or authenticate a public tag. Release installation guidance is pinned to v0.9.1. To inspect
...
fixture set. It tests generic update and migration behavior; it is not the current `0.9.1` candidate record. Never copy their trust key, pin
...
- Builder `0.9.0 -> 0.9.1` with semantic classification `patch`;
