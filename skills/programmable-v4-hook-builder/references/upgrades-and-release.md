...
This default reports the constants bundled with the checked-out Builder. The v0.10.1 release package reports
`channel: stable`, `publicationState: release-package`, and `publicationStateVerified: false`; these bundled constants
do not resolve or authenticate a public tag. Release installation guidance is pinned to v0.10.1. To inspect
...
| Release candidate | `assets/templates/release-candidate.example.json` | Binds the planned 0.10.1 transition but deliberately leaves source and release-artifact coordinates incomplete |
...
fixture set. It tests generic update and migration behavior; it is not the current `0.10.1` candidate record. Never copy their trust key, pin
...
0.10.1 planning example it declares:

- Builder `0.10.0 -> 0.10.1` with semantic classification `patch`;
