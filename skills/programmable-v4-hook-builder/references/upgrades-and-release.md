...
| Release candidate | `assets/templates/release-candidate.example.json` | Binds the planned 0.4.0 transitions but deliberately leaves source and release-artifact coordinates incomplete |
...
fixture set. It tests generic update and migration behavior; it is not the current `0.4.0` release record. Never copy their trust key, pin
...
The candidate's closed `plannedRelease` object binds the intended identity. For the packaged private 0.4.0 candidate it
declares:

- Builder `0.3.0 -> 0.4.0` with semantic classification `minor`;
- submission standard `1.4.0 -> 1.5.0`;
