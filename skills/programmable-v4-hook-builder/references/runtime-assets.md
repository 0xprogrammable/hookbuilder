...
dependencies, product surfaces, and independent source bindings. Application V3.1 carries the current generic
Applicant repository closure; its Fee V2 fields apply only when that exact legacy package was explicitly selected.
...
For current Application V3 transport, each repository closes through inline paths or a content-addressed source manifest. Large asset sets may
