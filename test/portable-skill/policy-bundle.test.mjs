...
import { normativePolicyInventory } from "../../skills/programmable-v4-hook-builder/scripts/submission-core.mjs";
...
test("normative manifest binds every policy, template, enforcement file and blind fixture", () => {
  ...
  try {
    ...
    assert.ok(policyPaths.includes("portable-package.json"));
    assert.ok(policyPaths.includes("scripts/installed-runtime-smoke.mjs"));
    assert.equal(policyPaths.some((policyPath) => policyPath.startsWith("scripts/test/")), false);
    assert.equal(policyPaths.some((policyPath) => policyPath.startsWith("test/portable-skill/")), false);
