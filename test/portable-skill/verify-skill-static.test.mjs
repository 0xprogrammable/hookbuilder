...
import { validateInstalledProvenance } from "../../skills/programmable-v4-hook-builder/scripts/verify-skill-provenance-core.mjs";
import { markdownHeadingAnchors, parseCanonicalYamlMapping } from "../../skills/programmable-v4-hook-builder/scripts/verify-skill-yaml-core.mjs";
...
test("source verification partitions every repository test exactly once with bounded fanout", () => {
  ...
  assert.equal(testFiles.length, 87);
  assert.deepEqual(batches.map((batch) => batch.length), [44, 43]);
...
test("installed-mode portable execution keeps its runtime smoke in one nonempty shard", async () => {
  const batches = createDeterministicTestBatches(["scripts/installed-runtime-smoke.mjs"]);
  assert.deepEqual(batches, [["scripts/installed-runtime-smoke.mjs"]]);
  let calls = 0;
...
function readDeclaredRequiredInventories() {
  ...
  const testDeclaration = executionSource.match(/REQUIRED_REPOSITORY_TESTS = Object\.freeze\(`([\s\S]*?)`\.trim\(\)\.split/u);
  assert.ok(testDeclaration, "source verifier must declare its complete repository test inventory");
  const repositoryTestPaths = testDeclaration[1]
    .trim()
    .split(/\s+/u)
    .map((stem) => `test/portable-skill/${stem}.test.mjs`);
  ...
  for (const rawLine of requiredDeclaration[1].split("\n")) {
    ...
    } else if (line === "INSTALLED_RUNTIME_SMOKE,") {
      requiredPaths.push("scripts/installed-runtime-smoke.mjs");
    } else {
  ...
  return { repositoryTestPaths, requiredPaths };
}
...
test("trusted verifier validates a candidate skill as data without executing its tests", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(candidateRoot, "scripts", "installed-runtime-smoke.mjs"),
...
test("source verifier declares every repository test exactly once", () => {
  const { repositoryTestPaths } = readDeclaredRequiredInventories();
  const discovered = fs.readdirSync(testDirectory)
    .filter((name) => name.endsWith(".test.mjs"))
    .sort()
    .map((name) => `test/portable-skill/${name}`);

  assert.equal(repositoryTestPaths.length, 87);
...
test("portable verifier deletion-guards its exact required inventory in one bounded probe", () => {
  ...
  try {
    fs.cpSync(skillRoot, candidateRoot, { recursive: true });
    const deletedRequiredPath = "references/agent-entry-and-application.md";
    fs.rmSync(path.join(candidateRoot, deletedRequiredPath));
    ...
    assert.deepEqual(emittedMissingDiagnostics, [`- missing ${deletedRequiredPath}`]);
    assert.match(result.stderr, new RegExp(`(?:^|\\n)- missing ${escapeRegExp(deletedRequiredPath)}(?:\\n|$)`, "u"));
  } finally {
    fs.rmSync(fixtureRoot, { recursive: true, force: true });
  }
});
...
test("portable verifier drain-preserves a complete diagnostic payload larger than a pipe buffer", () => {
  ...
  try {
    ...
    assert.match(result.stderr, /excluded repository-only files leaked into the portable package/u);
    for (const relativePath of undeclared) assert.match(result.stderr, new RegExp(escapeRegExp(relativePath), "u"));
  } finally {
    fs.rmSync(fixtureRoot, { recursive: true, force: true });
  }
});

test("portable verifier rejects a file outside the canonical package manifest", () => {
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "programmable-static-test-inventory-"));
  const candidateRoot = path.join(fixtureRoot, "programmable-v4-hook-builder");

  try {
    fs.cpSync(skillRoot, candidateRoot, { recursive: true });
    fs.writeFileSync(path.join(candidateRoot, "undeclared-portable.txt"), "not classified\n");

    const result = runUntrustedVerifier(candidateRoot);

    assert.notEqual(result.status, 0, result.stdout);
    assert.match(result.stderr, /portable files are outside the canonical inclusion manifest: undeclared-portable\.txt/u);
  } finally {
    fs.rmSync(fixtureRoot, { recursive: true, force: true });
  }
});
