...
test("platform CI routing is narrow for documentation and fail-closed for executable paths", () => {
  ...
  assert.deepEqual(executable.repositoryNodes, [24]);
  ...
  assert.deepEqual(unknown.repositoryNodes, [24]);
...
test("platform CI routing selects only relevant kernels on pull requests", () => {
  ...
  assert.deepEqual(v1.repositoryNodes, [24]);
  ...
  assert.deepEqual(codeowners.repositoryNodes, [24]);
...
test("protected and release CI routing retains the exhaustive compatibility lanes", () => {
  for (const input of [
    ...
    assert.deepEqual(routing.repositoryNodes, [24], `${input.event}:${input.ref}`);
