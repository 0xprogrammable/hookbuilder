...
test("active Markdown installs resolve only the intended immutable v0.10.3 release package", () => {
  ...
  for (const { path: documentPath, source, command } of installs) {
    assert.match(command, /(?:@v0\.10\.3\b|--pin\s+v0\.10\.3\b)/u, documentPath);
...
test("stable v0.5.1 through v0.10.2 history is immutable and v0.10.3 is the release package", () => {
  ...
  const candidateNotes = readText("docs/releases/v0.10.3.md");
  ...
  assert.equal(versionAuthority.version, "0.10.3");
  ...
  assert.deepEqual(candidate.changes.map(({ id, kind }) => ({ id, kind })), [
    { id: "tree-derived-blob-fetch-fix", kind: "bug-fix" }
  ]);
  ...
  assert.match(readme, /Release package `v0\.10\.3`/u);
  assert.match(readme, /`publicationStateVerified: false`/u);
  assert.match(readme, /--pin v0\.10\.3/u);
  ...
  assert.match(candidateNotes, /^# Programmable v4 Builder v0\.10\.3$/mu);
  assert.match(candidateNotes, /Exact-object resolver fix/u);
  assert.match(candidateNotes, /Regression proof/u);
  assert.match(candidateNotes, /Authority boundary/u);
  assert.match(candidateNotes, /`publicationStateVerified: false`/u);
  assert.match(releasing, /Current release-package and installation identity: `v0\.10\.3`/u);
  assert.match(releasing, /`publicationStateVerified: false`/u);
  assert.match(releasing, /Prior immutable releases: `v0\.10\.2`, `v0\.10\.1`, `v0\.10\.0`, `v0\.9\.3`, `v0\.9\.2`, `v0\.9\.1`, `v0\.9\.0`, `v0\.8\.0`, `v0\.7\.0`, `v0\.6\.0`, and `v0\.5\.1`/u);
...
test("v0.9.3 quantitative docs retain their exact inventories and one hash-bound release baseline", () => {
  ...
  assert.equal(sizeReport.portablePackage.bytes, 8_677_748);
