...
test("active Markdown installs resolve only the intended immutable v0.10.2 release package", () => {
  ...
  for (const { path: documentPath, source, command } of installs) {
    assert.match(command, /(?:@v0\.10\.2\b|--pin\s+v0\.10\.2\b)/u, documentPath);
...
test("stable v0.5.1 through v0.10.1 history is immutable and v0.10.2 is the release package", () => {
  ...
  const candidateNotes = readText("docs/releases/v0.10.2.md");
  const previousCandidateNotes = readText("docs/releases/v0.10.1.md");
  ...
  const previousCandidateSection = changelog.match(/^## 0\.10\.1[^\n]*\n[\s\S]*?(?=^## 0\.10\.0)/mu)?.[0];
  const priorCandidateSection = changelog.match(/^## 0\.10\.0[^\n]*\n[\s\S]*?(?=^## 0\.9\.3)/mu)?.[0];

  assert.equal(versionAuthority.version, "0.10.2");
  ...
  assert.deepEqual(candidate.changes.map(({ id, kind }) => ({ id, kind })), [
    { id: "policy-neutral-proposal-draft-fix", kind: "bug-fix" }
  ]);
  ...
  assert.equal(sha256Text(previousCandidateNotes), "04d19187ae49df64abc06bf0dc8d4e3f844a221a46899bb5150d1de32980d431");
  assert.equal(sha256Text(previousCandidateSection), "5745d0f8e9efd2123ef4d3ce4678f8441fb08e100b0a0633c9f548da95663534");
  ...
  assert.match(readme, /Release package `v0\.10\.2`/u);
  assert.match(readme, /`publicationStateVerified: false`/u);
  assert.match(readme, /--pin v0\.10\.2/u);
  ...
  assert.match(candidateNotes, /^# Programmable v4 Builder v0\.10\.2$/mu);
  assert.match(candidateNotes, /Policy-neutral Proposal Draft fix/u);
  ...
  assert.match(releasing, /Current release-package and installation identity: `v0\.10\.2`/u);
  assert.match(releasing, /`publicationStateVerified: false`/u);
  assert.match(releasing, /Prior immutable releases: `v0\.10\.1`, `v0\.10\.0`, `v0\.9\.3`, `v0\.9\.2`, `v0\.9\.1`, `v0\.9\.0`, `v0\.8\.0`, `v0\.7\.0`, `v0\.6\.0`, and `v0\.5\.1`/u);
...
test("v0.9.3 quantitative docs retain their exact inventories and one hash-bound release baseline", () => {
  ...
  assert.equal(sizeReport.portablePackage.bytes, 8_679_234);
